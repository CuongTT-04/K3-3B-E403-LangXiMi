...
khi chuẩn hoá khoảng trắng). Bộ đánh giá hồi quy (golden-set, 21 case) có
...
- **LLM:** `MockLLM` (deterministic, không gọi mạng, dùng trong test/eval,
  nhận `chosen` để chẩn đoán theo phương án đã chọn); `GeminiLLM`
  (`LLM_MODE=gemini`, REST qua `httpx`, KHÔNG BAO GIỜ raise — retry đúng 1
  lần khi `TimeoutException`/429/5xx, còn lại rơi thẳng về `MockLLM`); `RealLLM`
  là stub cho provider khác trong tương lai.
